...
IMAGE_PATH = 'Media/IMG_3085.JPG'
image = cv2.imread(IMAGE_PATH)
width = image.shape[1]
height = image.shape[0]
print(width, height)
# image = image.resize(800, int((800/width) * height))
img = cv2.resize(image, dsize=(1300, int((1300/width) * height)))
...
with open('description.txt', 'w') as f:
    ...
    for detection in result:
        ...
        f.write(text+"\n")
        img = cv2.rectangle(img, tuple([top_left[0]-30, top_left[1]-30]), tuple([bottom_right[0]+30, bottom_right[1]+30]), (0, 255, 0), 4)
